def main():
    ...
    while (resp != 0):
        ...
        print("9-Gravação dos dados de todos os funcionários em arquivo texto")
        ...
        if (opc == 1):
            ...
            if (len(lista_funcionarios) > 0):
                grava_arqtexto_todos(lista_funcionarios)
                print("Gravação concluída com sucesso!")
        elif (opc == 10):
            if (len(lista_funcionarios) > 0):
                grava_arqtexto_relatorio_1(lista_funcionarios)
                print("Gravação concluída com sucesso!")
        elif (opc == 11):
            if (len(lista_funcionarios) > 0):
                grava_arqtexto_relatorio_2(lista_funcionarios)
                print("Gravação concluída com sucesso!")
        else:
...
def grava_arqtexto_todos(lista_funcionarios):

    for i in range(len(lista_funcionarios)):
        texto = str(lista_funcionarios[i]['Codigo'])+" "+lista_funcionarios[i]['Nome']+" "+str(lista_funcionarios[i]['Idade'])+" "+str(lista_funcionarios[i]['Salario'])+"\n"
        with open("dados_todos_func.txt","a",encoding="UTF8") as arq:
            arq.write(texto)
            arq.close()

def grava_arqtexto_relatorio_1 (lista_funcionarios):
    bubbleSort(lista_funcionarios, "Idade")

    for i in range(len(lista_funcionarios)):
        if (lista_funcionarios[i]['Idade'] > 25):
            texto = str(lista_funcionarios[i]['Codigo']) + " " + lista_funcionarios[i]['Nome'] + " " + str(lista_funcionarios[i]['Idade']) + " " + str(lista_funcionarios[i]['Salario']) + "\n"
            with open("dados_relat1_func.txt", "a", encoding="UTF8") as arq:
                arq.write(texto)
                arq.close()

def grava_arqtexto_relatorio_2 (lista_funcionarios):
    bubbleSort(lista_funcionarios, "Salario")

    for i in range(len(lista_funcionarios)):
        if (lista_funcionarios[i]['Idade'] >= 22 and lista_funcionarios[i]['Idade'] <= 35 and lista_funcionarios[i]['Salario'] > 3700):
            texto = str(lista_funcionarios[i]['Codigo']) + " " + lista_funcionarios[i]['Nome'] + " " + str(lista_funcionarios[i]['Idade']) + " " + str(lista_funcionarios[i]['Salario']) + "\n"
            with open("dados_relat2_func.txt", "a", encoding="UTF8") as arq:
                arq.write(texto)
                arq.close()
